...
/**
 * @author Ray Mooney A simple backwardBigram language model that uses simple
...
public class BackwardBigramModel {
    ...
    public void trainSentence(List<String> sentence) {
        List<String> trainSentence = new ArrayList(sentence);
        Collections.reverse(trainSentence);
        ...
        for (String token : trainSentence) {
    ...
    /**
     * Compute unigram and backwardBigram probabilities from unigram and
     * backwardBigram counts
     */
    ...
    public void test(List<List<String>> sentences) {
        ...
        for (List<String> sentence : sentences) {
            List<String> testSentence = new ArrayList(sentence);
            Collections.reverse(testSentence);

            // Num of tokens in sentence plus 1 for predicting </S>
            totalNumTokens += testSentence.size() + 1;
            // Compute log prob of sentence
            double sentenceLogProb = sentenceLogProb(testSentence);
...
//          System.out.println("Test sentence in sentence log prob" + sentence );
    ...
    public void test2(List<List<String>> sentences) {
        ...
        double totalNumTokens = 0;
        for (List<String> sentence : sentences) {
            List<String> testSentence2 = new ArrayList(sentence);
            Collections.reverse(testSentence2);

            totalNumTokens += testSentence2.size();
            double sentenceLogProb = sentenceLogProb2(testSentence2);
            //	    System.out.println(sentenceLogProb + " : " + sentence);
            totalLogProb += sentenceLogProb;
        }
        double perplexity = Math.exp(-totalLogProb / totalNumTokens);
    ...
    /**
     * Interpolate backwardBigram prob using backwardBigram and unigram model
     * predictions
     */
...
//        args[1] = "0.1";
      ...
      testFraction = Double.valueOf(args[args.length - 1]);
        // Get list of sentences from the LDC POS tagged input files
       sentences = POSTaggedFile.convertToTokenLists(files);
